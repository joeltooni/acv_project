...
#     mask = rle_to_mask(best_det["segmentation"])
#     mask = np.asarray(mask, dtype=np.uint8)  # Explicit type conversion
...
#         # edge = canny(mask.astype(np.uint8))  # Canny expects 0-255 input
#         # edge = binary_dilation(edge, np.ones((2, 2)))
#         edge = canny(np.array(mask, dtype=np.uint8, copy=True))  # Force new contiguous array
#         edge = binary_dilation(edge, np.ones((2, 2)))
...
def visualize(rgb, detections, save_path="tmp.png"):
    # Convert and prepare base image
    ...
    # Bulletproof edge detection
    def get_edges(m):
        try:
            edges = cv2.Canny(m, 0, 1)  # Using OpenCV's Canny instead
            return edges > 0
        except Exception as e:
            print(f"Edge detection fallback: {e}")
    ...
    # Apply visualization
    color = [int(c*255) for c in colors[best_det["category_id"]-1]]
    alpha = 0.33
    
    for c in range(3):
        img[mask>0, c] = alpha*color[c] + (1-alpha)*img[mask>0, c]
    img[edge] = 255
...
def run_inference(segmentor_model, output_dir, cad_path, rgb_path, depth_path, cam_path, stability_score_thresh):
    ...
    print("Array dtype:", rgb_array.dtype)
    # detections = model.segmentor_model.generate_masks(np.array(rgb))
    # detections = Detections(detections)
    rgb_array = np.ascontiguousarray(np.array(rgb))  # Ensure memory layout is compatible
